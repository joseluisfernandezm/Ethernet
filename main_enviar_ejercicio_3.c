...
// Declaracion de Variables
	...
	uint16_t longitud_camposiguiente;//tam del mensa
	...
	if (resultado==-1)
	{
        ...
        memcpy(usuario,argv[2],strlen(argv[2]));//copiamos en uduario el nombre que hallamos escrito en argv[2] desde la terminal
...
//----------------cambios ejercicio 3----------------
	...
	for(int i = 0; i<200; i++) {
		for(int j = 0; j<1400; j++)
			cadena[j] = rand()%256;
	longitud_camposiguiente=1400;//bucle que genera 200 mensajes con cadenas aleatorias
	...
	/**PRACTICA Implementar aquí las acciones necesarias para crear un mensaje y un canal*/
		...
		memcpy(trama_chat+incremento, &currentTime.tv_sec, 20);//reservamos un tamaño de 20 caracteres para el tiempo
